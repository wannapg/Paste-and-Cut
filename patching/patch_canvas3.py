...
class patchROI:
    ...
    def recursive_packing(self,img, x,y,w,h,patched_image,result,i,sorted_lines,min_w, min_h,f,obj_size):
        ...
        if priority<5:
            ...
            if priority == 2:
                self.recursive_packing(img,x,y,w,h-d,patched_image,result,i,sorted_lines,min_w, min_h,f,obj_size)

            elif priority ==3:
                self.recursive_packing(img,x+omega,y,w-omega,h,patched_image,result,i,sorted_lines,min_w, min_h,f,obj_size)

            elif priority == 4:
                if w - omega < min_w :
                    self.recursive_packing(img,x,y+d,w,h-d,patched_image,result,i,sorted_lines,min_w, min_h,f,obj_size)

                elif h-d <min_h:
                    self.recursive_packing(img,x+omega,y,w-omega,h,patched_image,result,i,sorted_lines,min_w, min_h,f,obj_size)

                elif omega <min_w :
                    self.recursive_packing(img,x+omega,y,w-omega,d,patched_image,result,i,sorted_lines,min_w, min_h,f,obj_size)
                    self.recursive_packing(img,x,y+d,w,h-d,patched_image,result,1,sorted_lines,min_w, min_h,f,obj_size)

                else:
                    self.recursive_packing(img,x,y+d,omega,h-d,patched_image,result,i,sorted_lines,min_w, min_h,f,obj_size)
                    self.recursive_packing(img,x+omega,y,w-omega,h,patched_image,result,1,sorted_lines,min_w, min_h,f,obj_size)

        return patched_image
           
   
        
    def patch_input_algorithm(self,image_path,roi_info_path,save_patched_image_path,save_patched_info_path,obj_size):
            ...
            for file in roi_info_list:
                ...
                with open(file) as roi_info:
                    ...
                    for i,read_line in enumerate(sorted_lines):
                        ...
                        patched_image=self.recursive_packing(img,x,y,w,h,patched_image,result,i,sorted_lines,min_w, min_h,f,obj_size)
                        x,y = 0,0
...
def main_input():
    ...
    for i,value in enumerate(data):
        image_path = '/home/rtcl/workspace/mAP/input/images-optional/MOT16-'+value+'/'

        if i==0:
            roi_info_path='/home/rtcl/workspace/yolov5/runs/detect/1280merged/exp3/mapped_labels/1920_mapformat/'
        else:
            roi_info_path='/home/rtcl/workspace/yolov5/runs/detect/640merged/exp'+ str(i+1)+'/mapped_labels/'
        save_patched_image_path = '/home/rtcl/workspace/yolov5/patch/patch_algo/nopad_704/'+value+'/patched_image/'
        save_patched_info_path = '/home/rtcl/workspace/yolov5/patch/patch_algo/nopad_704/'+value+'/patched_info/'
        if not os.path.exists(save_patched_image_path):
            os.mkdir(save_patched_image_path)
        if not os.path.exists(save_patched_info_path):
            os.mkdir(save_patched_info_path)
        patch.patch_input_algorithm(image_path,roi_info_path,save_patched_image_path,save_patched_info_path,640)
